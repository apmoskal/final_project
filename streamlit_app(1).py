...
time_range = st.slider("Select Years",
                            int(df["Year"].dt.year.min()), 
                            int(df["Year"].dt.year.max()), 
                            ((int(df["Year"].dt.year.min()), int(df["Year"].dt.year.max()))))

df['Year'] = df['Year'].dt.year

# Selectbox: Filter by Country
# country_options = ["All"] + list(df["country"].unique())
# country = st.selectbox("Filter by Country", options=country_options)

# # Filter the DataFrame only if a specific country is selected
# if country != "All":
...
scatter = alt.Chart(filtered_df).mark_circle().encode(
   ...
   # size=alt.Size('food_supply:Q', title='Food Supply'),
    tooltip=['country:N', x_axis_choice + ':Q', 'hg/ha_yield:Q', 'Item:N', 'food_supply:Q']
).add_params(
    selection
).properties(
    width=500,
